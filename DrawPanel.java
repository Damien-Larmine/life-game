 ...
 * Par Damien Larmin� et Marine Megherbi
 * 
 * Jeu de la Vie : Cr�e par John Conway en 1970
 * 
 * Classe DrawPanel qui dessine les cases et teste leur voisinage pour d�terminer si elle vivent ou meurent
 * */
...
public class DrawPanel extends JPanel{
    ...
    public DrawPanel(final int cellSize, int cellCols, int cellRows ){
		...
		if(population == 0){
			...
			this.addMouseMotionListener(new MouseMotionListener(){ //listener qui dessine les cases lorsque l'on garde le bouton gauche cliqu�
    ...
    //M�thode qui dessine la grille et les cellules
    ...
    // M�thode qui calcule la g�n�ration suivante
    ...
    public synchronized void next() { 
		...
		// test les voisins des cellules sur les cot�s de la grille 
		x=1; 
		y=0; 
		int dx=1; 
		int dy=0; 
		while( true ) { 
			...
			// lorsque l'on atteint le cot� de la grille on change de direction
			if ( x==cellCols-1 && y==0 ) { 
				dx = 0; 
				dy = 1; 
			...
			// se termine quand toutes les cellules ont �t� test�es
				break; 
			} 
			x += dx; 
			y += dy; 
